...
export const pubkeyToAccount = (pubkey: string): string => {
  const hash256 = sha256().update(Buffer.from(pubkey, "hex")).digest();
  const hash160 = ripemd160().update(hash256).digest();
  ...
  const finish = (returnValue: string): number | boolean => {
    ...
    if (
      ...
      return (
        ...
        Number(
          (
            unsignedReturnValue.slice(2) +
            "0".repeat(83 - unsignedReturnValue.length)
          ).replace(/^0+/, "")
        )
      );
    }
...
export const nftValuetoXrpl = (
  ...
  if (
    ...
    xrplValueToNft(accountBalance) === false
  ) {
