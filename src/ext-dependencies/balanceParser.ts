...
import { AnyJson, FormattedBalanceChanges } from "../types";
...
export const parseBalanceChanges = (
  ...
  const formatted = Object.keys(quantities).reduce(
    (a: FormattedBalanceChanges, b: string): FormattedBalanceChanges => {
      const formattedQuantities = quantities[b].map((q) => {
        return Object.assign(q, {
          formatted: {
            value:
              q.counterparty !== "" && q.value.match(/e/)
                ? String(xrplValueToNft(q.value))
                : q.value,
            currency:
              q.currency === "XRP" && q.counterparty === ""
                ? "XRP"
